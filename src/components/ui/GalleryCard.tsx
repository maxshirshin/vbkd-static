...
export function GalleryCard({ slug, title, description, coverImage }: GalleryCardProps) {
  return (
    <a href={`/gallery/${slug}`} className="group block">
      {coverImage ? (
        <CDNImage
          srcPath={coverImage}
          alt={`${title} — cover`}
        ...
        <div className="w-full aspect-square mb-4 flex items-center justify-center transition-shadow group-hover:shadow-lg bg-border-light">
          <span className="text-sm text-text-light">Bild fehlt</span>
        </div>
      )}
      <h2 className="text-xl mb-1 font-heading text-primary-dark">{title}</h2>
