...
2. **Configure environment variables:**
   ...
   _Note: Ensure `VITE_CDN_URL` is set to the correct CDN URL to ensure images load properly in development (e.g. `http://www.verein-botanischekunst.de` or your `.gcdn.co` edge URL)._
...
1. **Build the static HTML/JS/CSS assets:**
   ...
   _This pre-renders all pages to `dist/client/` using Vike._
...
3. **Deploy to GCore Object Storage:**
   ...
   npm run deploy
   ```
   _This script reads your `.env` credentials, computes MIME types and aggressive Cache-Control headers, and pushes both your `dist/client/` output and local `images/` directory straight to the configured GCore Storage Bucket._
